        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < s; j++) {
                if (prtsize.get(j) >= processs[i]) {
                    ...
                    else if (prtsize.get(worstIdx) < prtsize.get(j)) {
                        worstIdx = j;
                    }
                }
            }
            if (worstIdx != -1)
            {
                proname.set(worstIdx, pro[i]);
                int remaining = prtsize.get(worstIdx) - processs[i];
                prtsize.set(worstIdx, processs[i]);

                if(remaining>0) {
                    prtsize.add(worstIdx + 1, remaining);
                    prtname.add(worstIdx + 1, "partition " + (s));
                    proname.add(worstIdx+1, "-1");
                }
            }
        }
        print();
